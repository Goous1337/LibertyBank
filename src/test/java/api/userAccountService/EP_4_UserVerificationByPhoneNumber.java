...
public class EP_4_UserVerificationByPhoneNumber extends BaseTest {
    ...
    public void userVerificationWithValidData() {
       ...
       userAccountService.getVerificationCode()
    ...
    public void userVerificationWithoutVerificationCode() {
        Response response = userAccountService.verificationMobilePhone(REGISTERED_PHONE_NUMBER);
        assertAll(
    ...
    public void userVerificationWithInvalidVerificationCode() {
        assertEquals(HttpStatus.SC_BAD_REQUEST, userAccountService.verificationMobilePhoneVerificationCode(REGISTERED_PHONE_NUMBER,
                        "111111").statusCode(),
                "Код ответа не соответствует ожидаемому");
    }
    ...
    public void userVerificationWithoutMobilePhone() {
        String verificationCode = userAccountService.getVerificationCode(REGISTERED_PHONE_NUMBER);
        assertEquals(HttpStatus.SC_BAD_REQUEST, userAccountService.verificationVerificationCode(verificationCode).statusCode(),
                "Код ответа не соответствует ожидаемому");
    }
    ...
    public void userVerificationWithInvalidData(String invalidMobilePhone, String invalidVerificationCode) {
        Response response = userAccountService.verificationInvalidMobilePhoneAndVerificationCode(invalidMobilePhone, invalidVerificationCode);
    ...
    public void userVerificationWithInvalidMethod() {
        String verificationCode = userAccountService.getVerificationCode(REGISTERED_PHONE_NUMBER);
        Response response = userAccountService.verificationWithInvalidMethod(REGISTERED_PHONE_NUMBER, verificationCode);
    ...
    public void userVerificationWithValidDataAfterBlocking(String invalidMobilePhone, String invalidVerificationCode) {
        ...
        assertEquals(HttpStatus.SC_NOT_ACCEPTABLE, response.getStatusCode(), "Код ответа не соответствует ожидаемому");
        String verificationCode = userAccountService.getVerificationCode(REGISTERED_PHONE_NUMBER);
        Response response1 = userAccountService.verificationMobilePhoneVerificationCode(REGISTERED_PHONE_NUMBER, verificationCode);
    ...
    public void userVerification() {
        Response response = userAccountService.verificationInvalidMobilePhoneAndVerificationCode(NOT_REGISTERED_PHONE_NUMBER,
                "111111");
        assertEquals(HttpStatus.SC_NOT_ACCEPTABLE, response.getStatusCode(), "Код ответа не соответствует ожидаемому");
        String verificationCode = userAccountService.getVerificationCode(REGISTERED_PHONE_NUMBER);
        Response response1 = userAccountService.verificationMobilePhoneVerificationCode(REGISTERED_PHONE_NUMBER, verificationCode);
